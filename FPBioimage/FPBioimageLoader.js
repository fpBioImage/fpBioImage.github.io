...
function CompatibilityCheck()
{
    ...
    if (!1 && !hasWebGL)
    {
        alert("FP Bioimage requires WebGL to run correctly. Try installing Chrome.");
        window.history.back();
    }
    // Show warnings if needed.
    else if (mobile)
    {
        if (!confirm("Please note that FP Bioimage is not fully supported on mobiles. Press Ok if you wish to continue anyway."))
            window.history.back();
    }
    else if (browser.indexOf("Firefox") == -1 && browser.indexOf("Chrome") == -1 && browser.indexOf("Safari") == -1)
    {
        if (!confirm("Please note that your browser is not currently supported for FP Bioimage. Try installing Chrome 64-bit, or press Ok if you wish to continue anyway."))
            window.history.back();
    }
}
...
if (typeof window.onerror != 'function')
{
    window.onerror = function UnityErrorHandler(err, url, line)
    {
        ...
        {
            alert ("Out of memory. We suggest upgrading to the latest 64-bit version of your browser.");
            return;
        }
        if (err.indexOf("Invalid array buffer length") != -1 || err.indexOf("out of memory") != -1 )
        {
            alert ("The browser could not allocate enough memory. We suggest upgrading to the latest 64-bit version of your browser.");
            return;
        }
        if (err.indexOf("Script error.") != -1 && document.URL.indexOf("file:") == 0)
        {
            alert ("It seems your browser does not support running FP Bioimage content from file:// urls. Please upload it to an http server, or try a different browser.");
            return;
        }
        alert ("An error occured running FP Bioimage on this page. See your browser's JavaScript console for more info. The error was:\n"+err);
    }
}
...
(function() {
 var loadPackage = function(metadata) {
    ...
    var PACKAGE_NAME = 'FPBioimage.data';
    var REMOTE_PACKAGE_BASE = 'FPBioimage.data';
    ...
    var REMOTE_PACKAGE_NAME = typeof Module['locateFile'] === 'function' ?
      ...
      var REMOTE_PACKAGE_SIZE = 12742807;
      var PACKAGE_UUID = '6d93aee2-343d-4818-9cdb-e6245b84fcc0';
...
Module['FS_createPath']('/Managed/mono', '2.0', true, true);
    ...
    DataRequest.prototype = {
      ...
      new DataRequest(0, 22924, 0, 0).open('GET', '/globalgamemanagers');
    new DataRequest(22924, 36140, 0, 0).open('GET', '/globalgamemanagers.assets');
    new DataRequest(36140, 180928, 0, 0).open('GET', '/level0');
    new DataRequest(180928, 189796, 0, 0).open('GET', '/level1');
    new DataRequest(189796, 218004, 0, 0).open('GET', '/level2');
    new DataRequest(218004, 218023, 0, 0).open('GET', '/methods_pointedto_by_uievents.xml');
    new DataRequest(218023, 221742, 0, 0).open('GET', '/preserved_derived_types.xml');
    new DataRequest(221742, 3022142, 0, 0).open('GET', '/resources.assets');
    new DataRequest(3022142, 3540754, 0, 0).open('GET', '/sharedassets0.assets');
    new DataRequest(3540754, 3545090, 0, 0).open('GET', '/sharedassets1.assets');
    new DataRequest(3545090, 9281622, 0, 0).open('GET', '/sharedassets2.assets');
    new DataRequest(9281622, 11157082, 0, 0).open('GET', '/Il2CppData/Metadata/global-metadata.dat');
    new DataRequest(11157082, 12654454, 0, 0).open('GET', '/Resources/unity_default_resources');
    new DataRequest(12654454, 12715182, 0, 0).open('GET', '/Resources/unity_builtin_extra');
    new DataRequest(12715182, 12742807, 0, 0).open('GET', '/Managed/mono/2.0/machine.config');
    ...
    function processPackageData(arrayBuffer) {
      ...
      DataRequest.prototype.byteArray = byteArray;
          ...
          Module['removeRunDependency']('datafile_FPBioimage.data');

    };
    Module['addRunDependency']('datafile_FPBioimage.data');
